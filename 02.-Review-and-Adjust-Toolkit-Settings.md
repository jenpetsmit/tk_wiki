...
The _vdb-config_ tool has a user interface used for adjusting Toolkit settings. See Figure. 

Alternatively, you can use a terminal window and type [vdb-config- ADD LINK To APPENDIX WITH COMMANDS](/vdb-config-commmands.html) commands to adjust settings.  
...
* Where to store downloaded file: **_user-repository_** cache

* Where to convert data from one format to another: **_process-local_** cache

If you need more information on selecting a location for _user-repository_ cache or _process-local_ cache, see [Things to Know](wiki.html#Things-to-know) for more information.  
...
**Note:** The _enable local file-caching_ option is always pre-selected. It should _only_ be deselected in special cases (e.g., compute-clusters when a common location cannot be configured).  To deselect, press the **i** key.
...
1. Press the **O key** to set _user-repository_ location. The _select directory_ window displays. 
...
If a location is not identified for the _process-local_ cache in Toolkit settings, Toolkit will likely download to current working directory.  The _fasterq-dump tool_ will warn you if it estimates there is not enough space. 


**To set the _process-local_ cache location:**

1. From the _Cache_ tab, press the **e key** to set _process-local_ location. The _select directory_ window displays. 
...
Note: This section presents information on cloud-based virtual machines (VMs) and buckets. To learn more, see [SRA in the Cloud](https://www.ncbi.nlm.nih.gov/sra/docs/sra-cloud/) for more information.
...
1. Press the **A key** to select AWS or press the **G key** to select GCP 
...
3. Press the **R key** to toggle selecting or deselecting the _report cloud instance identity_ option 
     * Reporting your cloud instance identity enables Toolkit to provide data from a location that is free and as local to your account as possible. If SRA does not know the location of your Cloud instance, the system will download from NCBI or AWS ODP, which could be significantly slower. 
...
4. Save  your _Cloud Access Credentials_ file to your local hardware or to your VM
     * To learn how to collect your credentials file, see [Collect Cloud Access Credentials **<-- NEEDS LINK to APPENDIX**](PAGE.HTML). 
